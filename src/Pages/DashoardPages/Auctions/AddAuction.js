...
const AddAuction = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      if (startDateObj < today) {
        ...
        setStartDate('');
        toast.error("Starting Date Should Be After Today's Date");
      } else if (startDateObj > new Date(endDate)) {
        // Check if the end date is before the start date
        setEndDate('');
        toast.error('Ending Date Should Be After Starting Date');
      } else {
        try {
          const auctionData = {
            name,
  ...
  const nameFeedback ='Please Enter Valid Name';
  const referenceNumberFeedback = 'Please Enter Valid Reference Number';
  const startDateFeedback = 'Please Enter A Valid Start Date';
  const endDateFeedback = 'Please Enter A Valid End Date';
  const timeFeedback = 'Please Enter A Valid Time';
  const feesFeedback = 'Please Enter A Valid Insurance Value';
  ...
  return (
    <div>
      ...
      >
        ...
        <CCol md={6}>
          <CFormLabel htmlFor="fees">Insurance</CFormLabel>
